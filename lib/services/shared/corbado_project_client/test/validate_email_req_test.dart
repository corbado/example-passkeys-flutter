//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:backend_api_public/api.dart';
import 'package:test/test.dart';

// tests for ValidateEmailReq
void main() {
  // final instance = ValidateEmailReq();

  group('test ValidateEmailReq', () {
    // Email to validate
    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // perform SMTP check for domain
    // bool smtpCheck
    test('to test the property `smtpCheck`', () async {
      // TODO
    });

    // enables domain suggestion for misspelled domains
    // bool suggestDomain
    test('to test the property `suggestDomain`', () async {
      // TODO
    });

    // Unique ID of request, you can provide your own while making the request, if not the ID will be randomly generated on server side
    // String requestID
    test('to test the property `requestID`', () async {
      // TODO
    });

    // ClientInfo clientInfo
    test('to test the property `clientInfo`', () async {
      // TODO
    });
  });
}
