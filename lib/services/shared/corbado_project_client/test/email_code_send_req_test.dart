//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:backend_api_public/api.dart';
import 'package:test/test.dart';

// tests for EmailCodeSendReq
void main() {
  // final instance = EmailCodeSendReq();

  group('test EmailCodeSendReq', () {
    // Recipient email address
    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // Defines if user email should be created if not found
    // bool create
    test('to test the property `create`', () async {
      // TODO
    });

    // Defines the lifetime of the token that needs to be validated
    // String tokenLifetime
    test('to test the property `tokenLifetime`', () async {
      // TODO
    });

    // Optional user's full name to be used if the user wasn't found and needs to be created first
    // String userFullName
    test('to test the property `userFullName`', () async {
      // TODO
    });

    // Template name of email to send
    // String templateName
    test('to test the property `templateName`', () async {
      // TODO
    });

    // Additional payload in JSON format
    // String additionalPayload
    test('to test the property `additionalPayload`', () async {
      // TODO
    });

    // Unique ID of request, you can provide your own while making the request, if not the ID will be randomly generated on server side
    // String requestID
    test('to test the property `requestID`', () async {
      // TODO
    });

    // ClientInfo clientInfo
    test('to test the property `clientInfo`', () async {
      // TODO
    });


  });

}
