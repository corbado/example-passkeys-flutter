...
class UserEmailGetRsp {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UserEmailGetRsp &&
  ...
  @override
  String toString() => 'UserEmailGetRsp[httpStatusCode=$httpStatusCode, message=$message, requestData=$requestData, runtime=$runtime, data=$data]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UserEmailGetRsp[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UserEmailGetRsp[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UserEmailGetRsp> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<UserEmailGetRsp>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UserEmailGetRsp>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UserEmailGetRsp.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
