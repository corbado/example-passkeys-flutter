...
class TrackingBackupStateGetRspAllOf {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TrackingBackupStateGetRspAllOf &&
     other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TrackingBackupStateGetRspAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TrackingBackupStateGetRspAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<TrackingBackupStateGetRspAllOf> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<TrackingBackupStateGetRspAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TrackingBackupStateGetRspAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TrackingBackupStateGetRspAllOf.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
