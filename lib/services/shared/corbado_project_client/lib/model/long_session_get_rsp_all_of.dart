...
class LongSessionGetRspAllOf {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is LongSessionGetRspAllOf &&
     other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "LongSessionGetRspAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "LongSessionGetRspAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<LongSessionGetRspAllOf> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<LongSessionGetRspAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<LongSessionGetRspAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = LongSessionGetRspAllOf.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
