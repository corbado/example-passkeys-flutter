...
class UserCreateRspAllOf {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is UserCreateRspAllOf &&
     other.data == data;
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "UserCreateRspAllOf[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "UserCreateRspAllOf[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<UserCreateRspAllOf> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<UserCreateRspAllOf>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<UserCreateRspAllOf>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = UserCreateRspAllOf.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
