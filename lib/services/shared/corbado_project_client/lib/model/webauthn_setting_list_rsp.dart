...
class WebauthnSettingListRsp {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is WebauthnSettingListRsp &&
  ...
  @override
  String toString() => 'WebauthnSettingListRsp[httpStatusCode=$httpStatusCode, message=$message, requestData=$requestData, runtime=$runtime, rows=$rows, paging=$paging]';
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "WebauthnSettingListRsp[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "WebauthnSettingListRsp[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<WebauthnSettingListRsp> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<WebauthnSettingListRsp>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<WebauthnSettingListRsp>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = WebauthnSettingListRsp.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
