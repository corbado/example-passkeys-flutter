...
> :warning: **Passkeys binding and subdomain as CNAME**: If the CNAME you enter in Corbado developer panel is a subdomain, e.g. `auth.your-domain.com`, you need to host the `assetlinks.json` file on the root / top-level domain: host `assetlinks.json` at `your-domain.com/.well-known/assetlinks.json`, as your relying party ID is the root / top-level domain `your-domain.com`.
